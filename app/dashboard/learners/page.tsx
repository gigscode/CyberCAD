...
import { Users, AlertCircle, UserCheck, Search, Flag } from 'lucide-react';
  ...
  const fetchLearners = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*, enrolments(id, status, courses(name)), learner_progress(current_score, last_activity_date)')
        .eq('role', 'learner')
